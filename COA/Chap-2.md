...
**定点小数**：表示纯小数，小数点位于符号位之后，数值最高位之前

**定点整数**：表示纯整数，小数点位于数制最低位之后

#### 四种码制的表示
**原码：** 正负数只在符号位不同，数值部分相同（表示数的绝对值）
$n+1$ 位原码整数，表示范围 $[-(2^n-1),2^n-1]$

**反码：** 正数与原码相同，负数数值部分按位取反
> 只是转化为补码的过程，实际无作用

**补码：** 正数与原码相同，负数数值部分按位取反再 $+1$
范围 $[-2^n,2^n-1]$
...
#### 一位全加器
逻辑结构：
![](./img/Chap-2/一位全加器.png)

本位和 $S_i = A_i \oplus B_i \oplus C_i$
进位 $C_i = A_i B_i + (A_i \oplus B_i)C_{i-1}$
> 或者 $C_i = A_i B_i + (A_i + B_i) C_{i-1}$ 只需要保证输入有 $\ge 2$ 的 $1$ 即可，但是如图复用了一个异或门，可以减少一个部件
...
![](./img/Chap-2/串行进位加法器.png)

#### 并行进位加法器*
并行进位的并行加法器
![](./img/Chap-2/并行进位加法器.png)

#### 带标识位的加法器
![](./img/Chap-2/带标识位的加法器.png)

#####
#### 算术逻辑单元ALU
ALU是一种功能较强的组合逻辑电路，能执行多种算数与逻辑运算，其中加法和减法有带标志加法器直接完成；乘除法则通常通过ALU配合控制逻辑，以多次加减和移位迭代完成，ALU还能执行与或非等逻辑运算
![](./img/Chap-2/ALU.png)
...
##### 无符号数
**$ZF$ 零标志位**和 **$CF$ 进/借标志位判断大小关系**

执行运算 $A-B$，根据 $ZF$ 和 $CF$ 的值即可判断
...
##### 有符号数
**$ZF$ 零标志位**，**$OF$ 溢出标志**，**$SF$ 符号标志**共同用于判断大小

也执行 $A - B$，根据 $ZF, OF, SF$ 值进行判断

-   **若 $A = B$**，则 $ZF = 1, OF = 0, SF = 0$
-   **若 $A > B$**
    -   若结果**无溢出**，则 $ZF = 0, OF = 0, SF = 0$
    -   若结果**溢出**，则 $ZF = 0, OF = 1, SF = 1$
-   **若 $A < B$**
    -   若结果**无溢出**，则 $ZF = 0, OF = 0, SF = 1$
    -   若结果**溢出**，则 $ZF = 0, OF = 1, SF = 0$
...
**执行过程（$n$ 轮）**
-   将乘数寄存器 $Y$ 的最低位送入控制逻辑进行判断
-   若该位为 $1$，执行加法（$P$ 与 $X$），运算结果写回 $P$，进位写入 $C$
    否则不操作
-   将 $C,P,Y$ 整体右移一位
-   计数器 $C_n$ 减 $1$，若 $C_n \ne 0$，则执行下一轮
...
**执行过程（重复 $n$ 轮）**
-   **判断** 将 $[Y]_补$ 最低位 $y_0$ 与辅助位 $y_{-1}$ 组合形成两位二进制码，送入控制逻辑
-   **加减法**
    -   对于 $10$，执行减法，将 $[-X]_补$ 置于 $P$
        > $-(Y_{n-1} - Y_n) = -1$
    -   对于 $01$，执行加法，将 $[X]_补$ 置于 $P$
        > $-(Y_{n-1} - Y_n) = 1$
    -   对于 $00/11$，不操作
...
### 定点数除法运算
#### 无符号整数的除法运算（恢复余数法）
![](./img/Chap-2/无符号整数除法电路（恢复余数法）.png)
...
$2n \ bit \div n \ bit \to$ 双精度除法，若第一位商 $1$，则发生商溢出
...
##### 异常处理
当检测到**除数为 $0$**或发生**商溢出**时，触发器立即停止工作，并置相应异常标志位

对于两个 $n$ 位补码除法，**商溢出**仅有一种情况：被除数最大负数 $-2^{n-1}$，且除数为 $-1$，此时结果 $2^{n-1}$ 无法用 $n$ 位补码表示

## 浮点数的表示与运算
IEEE754标准的浮点数格式
> C的 `float` 和 `double` 都是IEEE754标准浮点数格式

![](./img/Chap-2/科学计数法.png)

**规格化：** 确保尾数的最高位非零数位刚好在小数点之前
-   **规格化的目的**：提升精度
    > 具体的，对于相同的数据，如果不规格化可以表示为 $0.000101$ 或者 $1.01$，显然节约了尾数
...
![](./img/Chap-2/规格化浮点数取值范围.png)
也就是
$x \in [-(2-2^{23}) \times 2^{127},-1.0\times 2^{-126}] \cap [1.0\times 2^{-126},(2-2^{-23})\times 2^{127}]$
...
**下溢处理**
正负下溢统称为**下溢**
-   若结果落入非规格化区间，用非规格化浮点数存储,最小到 $\pm 1.0\times 2^{-149}$;
    若结果太小，按 $0$ 存储
...
##### C结构体对其规则
**规则**
-   每个成员的起始地址必须是其**对齐值**的整数倍
-   整个结构体大小必须是其**最大成员对其值**的整数倍
######