...
##### 单模光纤 -L
光纤的直径减小到只有一个光的波长，使得广播一直向前传播，不会产生多次反射，不会出现**脉冲展宽**。适合**长距离传输**并且**衰减更小**。
...
数据表示在每一个码元的开始处是否有跳变：**无跳变为bit1，有跳变为bit0**
...
1948年香农根据信息论理论推导出频率带宽首先且有高斯白噪声干扰的信道极限信息传输速率，即**香农公式**：$$ C = W \cdot \log_{2}(1 + \dfrac{S}{N})(b/s) $$
