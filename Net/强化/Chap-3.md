...
得到的 **$r$ 位余数**作为**冗余码**，加在原始数据之后形成**CRC校验码**
...
以 $n = 3$ 为例，编号以 $0 \to 2^n - 1$ 循环使用

|0|1|2|3|4|5|6|7|0|1|2|$\cdots$|
|-|-|-|-|-|-|-|-|-|-|-|-|

**给帧的编号由 $n$ 决定，和发送和接收窗口的长度无关**
######
...
IEEE 802.1 标准规定，发送帧的站点一旦**检测到产生了碰撞**，除了立即停止发送，还要在继续**发送至少 $32$ bit**（全 $1$ 或全 $0$）的**人为干扰信号**
...
#### 以太网MAC帧格式
有以太网V2和 IEEE 802.1两种标准，差别很小，目前市场上流行以太网V2的MAC帧

##### 以太网V2MAC帧格式
![](../img/Chap-3/以太网V2MAC帧.png)
...
![](../img/Chap-3/IEEE802.1Q帧.png)

**VLAN标签**占 $4B$，
-   前 $2B$ 是**标签协议标识符**，值固定为 `0x8100`，表示该帧为 IEEE 802.1Q 帧
    > 相当于占用了原本类型字段的位置
...
-   因为一般真是主机封装，再由交换机插入VLAN标签，若是主机之间封装出IEEE 802.1Q 帧，则最小长度可以减小到 $42B$，但这种情况很少
###### 
...
**接收帧**：只接受**未插入**VLAN标签字段的普通以太网MAC帧，根据接收帧的接口PVID给帧插入 $4B$ 的VLAN标签字段，使之成为 IEEE 802.1Q 帧，**标签的VID值即接口PVID值**

**转发帧**：若 IEEE 802.1Q 帧中VID的值域PVID相同，则删除VLAN标签，使之成为普通以太网MAC帧然后转发；若不同则不转发
...
**接收帧**：可接收IEEE 802.1Q 帧；也可接收普通的MAC帧。当接收普通以太网MAC帧，则根据帧接口的PVID给帧插入 $4B$ 的VLAN标签，使之成为 IEEE 801.1Q帧

**转发帧**：若IEEE 802.1Q 帧中的VID值与接口PVID值相同，则删除帧的VLAN标签，使之成为普通帧再转发；若不同，则直接转发

### 高速以太网

## **$\star \star \star$ 无线局域网WLAN**
> 曾考多次
...
|有固定基础设施的组网方式|描述|
|-|-|
|![](../img/Chap-3/有固定设施组网方式.png)|采用**星形网络拓扑**<br>位于其中心的**基站**被称为**接入点AP**|
...
> 可简单认为：地址1是当前帧接收者的地址，地址2是当前帧发送者的地址
...
##### 两种不同的媒体接入方式
**分布式协调功能DCF**
在DCF方式下，没有中心控制站点，每个站点使用CSMA/CA协议通过争用信道来获取发送权。**DCF方式是802.11定义的默认方式（必须实现）**
...
#### 退避算法
##### 与CSMA/CD的区别
当某个要发送帧的站点检测到无线信道从忙状态变为空闲状态时，除了要等待一个DIFS的时间，**还有退避一段山语湖时间**。因为可能有若干站点都立即发送帧，而CSMA/CA没有碰撞检测机制

##### 退避的执行
在执行退避时（等待DIFS的间隔后），站点会给自己的**退避计时器**设置一个随机的退避时间
-   每个空闲时隙，退避时间 $-1$
-   当退避时间减小到 $0$，就发送帧
-   当未减小到 $0$ 而信道有**转未忙状态**，就**冻结**退避计时器的退避时间
    重新等待空闲，然后经过DIFS间隔时间，继续**启动退避计时器**
...
随机数 $r \in \{0,1,\cdots,(2^{4+i}-1)\}$

初始 $i = 0$，每次重传 $i + 1$，最大为 $6$，此时r取值范围 $0 \sim 1023$

#### 信道预约机制
-   **源站**在发送数据帧之前，先发送一个段的控制帧，称为**请求发送RTS**帧
    需要检测到信道空闲一个DIFS时间才能发送
-   **目的站**正确收到源站发送的RTS帧，则等待一个SIFS时间发送一个短的控制帧作为响应，称为**允许发送CTS**帧
-   **源站**正确收到目的站发来的CTS帧，再等待一个SIFS时间就可以发送**数据帧**了

**若RTS帧发生碰撞**，源站不可能收到CTS帧，则**会执行退避算法重传RTS帧**

RST帧、CTS帧、数据帧、ACK帧都包含**持续期**字段，其他站只要监听七日任何一个，就知道信道被占用的时间。这样可以**减少隐蔽站带来的碰撞问题**

IEEE 802.11标准为用户提供了三种选择：
-   无条件使用RTS帧和CTS帧进行信道预约
-   只有当数据帧的长度超过了某个数据才使用RTS帧和CTS进行信道预约
-   不使用
#####

## 介质访问控制
又称**媒体访问控制**或**媒体接入控制MAC**，分为静态划分信道、随机访问、轮询访问三类
...
**要点**:
-   **核心思想**：预先分配信道资源，确保无碰撞
-   **技术实现**：频分复用FDM，时分复用TMD，码分复用CDM，波分复用WDM
-   **优点**：无碰撞，适用于稳定连续的通信
-   **缺点**：预先分配信道资源导致灵活性差
...
**要点**：
-   **核心思想**：竞争信道的使用权，产生碰撞后采用某种机制解决
-   **技术实现**：
    -   ALOHA协议
    -   CSMA协议
    -   CSMA/CD协议
    -   CSMA/CA协议
-   **优点**：灵活，适合（计算机）突发性数据的传送
-   **缺点**：高负载时性能下降，碰撞产生额外使用

#### ALOHA协议
**早期**计算机网络用语**共享信道的随机访问协议**

#### CSMA协议
> 曾考 $2$ 次

在ALONE协议基础上**增加了载波监听机制**，即**发送数据之前首先检测信道是否空闲**，若检测到信道，**根据策略**（如非坚持，1-坚持，P-坚持）**延时发送**。这样就避免了信道被其他站点占用的情况下盲目地发送数据的情况**

##### 非坚持CSMA
在发送数据之前需要监听信道
-   若信道空闲，立即发送数据
-   若信道忙，继续监听，**等待一个随机时间**后重新监听
-   **优点**：降低了多个站点等待信道空闲后同时发送数据碰撞的概率
-   **缺点**：可能因等待随机事件而延迟发送
-   **应用场景**：高负载，高延迟
######
...
在发送数据之前需要监听信道
-   若信道空闲，立即发送数据
-   若信道忙，继续监听，**直到空闲**后**立即发送**
-   **优点**：信道利用率高
-   **缺点**：同时发送碰撞概率大
-   **应用场景**：低负载，低延迟。共享式以太网（CSMA/CD）通常基于1-坚持CSMA
######
...
-   若信道空闲，进入**概率发送决策阶段**
    -   以概率 $P$ 发送数据
    -   以概率 $1-P$ 延迟发送，等待一个固定时隙后重新监听
-   若信道忙，继续监听到空闲，进入概率发送决策阶段
-   **优点**：通过调整概率P可以平衡概率与延迟
    -   $P = 1$ 退化为 1-坚持CSMA
    -   $P = 0$，退化为非坚持CSMA
-   **缺点**：$P$ 值确定困难，需要根据实际网络状态优化
-   **应用场景**：需要灵活的场景，IEEE 802.11 无线局域网（CSMA/CA）基于P坚持并结合退避机制
######
### 轮训访问
